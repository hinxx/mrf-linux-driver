...
static struct pci_device_id evr_ids[] = {
    ...
    {.vendor = PCI_VENDOR_ID_MRF, .device = PCI_DEVICE_ID_MRF_CPCIEVR300, .subvendor = PCI_VENDOR_ID_MRF, .subdevice = PCI_DEVICE_ID_MRF_CPCIEVR300},
    ...
    {.vendor = PCI_VENDOR_ID_XILINX, .device = PCI_DEVICE_ID_ZOMOJO_Z1, .subvendor = PCI_VENDOR_ID_XILINX, .subdevice = PCI_DEVICE_ID_ZOMOJO_Z1},
    {.vendor = PCI_VENDOR_ID_XILINX, .device = PCI_DEVICE_ID_0505, .subvendor = PCI_VENDOR_ID_MRF, .subdevice = PCI_DEVICE_ID_MRF_PXIEEVR300},
    {.vendor = PCI_VENDOR_ID_XILINX, .device = PCI_DEVICE_ID_KINTEX7, .subvendor = PCI_VENDOR_ID_MRF, .subdevice = PCI_DEVICE_ID_MRF_MTCAEVR300},
    {.vendor = PCI_VENDOR_ID_XILINX, .device = PCI_DEVICE_ID_KINTEX7, .subvendor = PCI_VENDOR_ID_MRF, .subdevice = PCI_DEVICE_ID_MRF_PCIEEVR300},
    {
        0,
    }};
    ...
    if (ev_device == NULL) {
        ...
        for (i = 0; i < MAX_MRF_BARS; i++) {
            if (ev_device->BAR_mmapped[i] != NULL) {
                ...
                release_mem_region(ev_device->BAR_start[i], ev_device->BAR_end[i] - ev_device->BAR_start[i] + 1);
            }
            ev_device->BAR_mmapped[i] = NULL;
        }
...
int ev_assign_irq(struct mrf_dev *ev_device) {
    ...
    result = request_irq(ev_device->irq, &ev_interrupt,
