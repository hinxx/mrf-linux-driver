...
int ev_open(struct inode *inode, struct file *filp) {
    ...
    if ((ev_device->access_device > 0 && ev_device->access_device < DEVICE_EV) || (ev_device->access_device == DEVICE_EV && minor != DEVICE_EV)) {
        printk(KERN_WARNING DEVICE_NAME ": open device major %d minor %d"
    ...
    if (minor >= DEVICE_FIRST && minor <= DEVICE_LAST) {
        ...
        case DEVICE_EEPROM:
            if (ev_device->devtype & (MRF_DEVTYPE_V2P_9030 | MRF_DEVTYPE_V5_9056)) {
                ...
                ev_device->eeprom_data = kmalloc(EEPROM_DATA_ALLOC_SIZE, GFP_KERNEL);
               ...
               to read in the data from the EEPROM. */
                if (ev_device->access_mode == O_RDONLY) {
                    if (ev_device->devtype & MRF_DEVTYPE_V2P_9030)
                        result = eeprom_9030_sprint(ev_device->pLC, ev_device->eeprom_data, EEPROM_DATA_ALLOC_SIZE);
                    if (ev_device->devtype & MRF_DEVTYPE_V5_9056)
                        result = eeprom_9056_sprint(ev_device->pLC, ev_device->eeprom_data, EEPROM_DATA_ALLOC_SIZE);
                    if (result < 0) {
        ...
        case DEVICE_FLASH:
            if (ev_device->devtype & MRF_DEVTYPE_V2P_9030) {
                ev_device->xcf_data = kmalloc(XCF_DATA_ALLOC_SIZE, GFP_KERNEL);
               ...
               sequence */
                ...
                ev_device->xcf_data = kmalloc(FLASH_DATA_ALLOC_SIZE, GFP_KERNEL);
        ...
        case DEVICE_FPGA:
            if (ev_device->devtype & (MRF_DEVTYPE_V2P_9030 | MRF_DEVTYPE_V5_9056)) {
                /* Allocate and clear memory for FPGA data */
                ev_device->fpga_conf_data = kmalloc(FPGA_DATA_ALLOC_SIZE, GFP_KERNEL);
               ...
               not supported */
                ...
                if (ev_device->access_mode == O_RDONLY) {
                    if (ev_device->devtype & MRF_DEVTYPE_V2P_9030)
                        result = fpga_9030_sprint(ev_device, ev_device->fpga_conf_data, FPGA_DATA_ALLOC_SIZE);
                    else if (ev_device->devtype & MRF_DEVTYPE_V5_9056)
                        result = fpga_9056_sprint(ev_device, ev_device->fpga_conf_data, FPGA_DATA_ALLOC_SIZE);
...
int ev_release(struct inode *inode, struct file *filp) {
    ...
    case DEVICE_EEPROM:
        if (ev_device->devtype & (MRF_DEVTYPE_V2P_9030 | MRF_DEVTYPE_V5_9056)) {
            if (ev_device->access_mode == O_WRONLY) {
                if (ev_device->devtype & MRF_DEVTYPE_V2P_9030)
                    result = eeprom_9030_sscan(ev_device->pLC, ev_device->eeprom_data);
                if (ev_device->devtype & MRF_DEVTYPE_V5_9056)
                    result = eeprom_9056_sscan(ev_device->pLC, ev_device->eeprom_data);
            }
    ...
    case DEVICE_FLASH:
        if (ev_device->devtype & MRF_DEVTYPE_V2P_9030) {
            ...
            if (ev_device->access_mode == O_WRONLY) {
                /* Flush buffer */
                jtag_9030_XCF_write(ev_device, ev_device->xcf_data, ev_device->xcf_data_pos, XCF_DATA_ALLOC_SIZE);
        ...
        if (ev_device->devtype & (MRF_DEVTYPE_V5_9056)) {
            ...
            if (ev_device->access_mode == O_WRONLY) {
                /* Flush buffer */
                flash_pageprogram(ev_device, ev_device->xcf_data, ev_device->xcf_data_pos, FLASH_DATA_ALLOC_SIZE);
        ...
        if (ev_device->devtype & (MRF_DEVTYPE_ECP3_PCI | MRF_DEVTYPE_ECP3_PCIE)) {
            ...
            if (ev_device->access_mode == O_WRONLY) {
                /* Flush buffer */
                flash_pageprogram(ev_device, ev_device->xcf_data, ev_device->xcf_data_pos + FLASH_SECTOR_PRIMARY_START, FLASH_DATA_ALLOC_SIZE);
            }
...
ssize_t ev_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos) {
    ssize_t retval = 0;
    struct mrf_dev *ev_device = (struct mrf_dev *)filp->private_data;
    ...
    case DEVICE_FLASH: {
        if (ev_device->devtype & MRF_DEVTYPE_V2P_9030) {
            if ((*f_pos & ~(XCF_DATA_ALLOC_SIZE - 1)) != ev_device->xcf_data_pos) {
                jtag_9030_XCF_read(ev_device, ev_device->xcf_data, (unsigned int)(*f_pos & ~(XCF_DATA_ALLOC_SIZE - 1)), XCF_DATA_ALLOC_SIZE);
                ev_device->xcf_data_pos = *f_pos & ~(XCF_DATA_ALLOC_SIZE - 1);
            }

            if (*f_pos + count > ev_device->xcf_data_pos + XCF_DATA_ALLOC_SIZE)
        ...
        if (ev_device->devtype & (MRF_DEVTYPE_V5_9056)) {
            ...
            if (*f_pos + count > ev_device->xcf_data_pos + FLASH_DATA_ALLOC_SIZE)
...
ssize_t ev_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos) {
    ...
    case DEVICE_FPGA:
        if (ev_device->devtype & (MRF_DEVTYPE_V2P_9030 | MRF_DEVTYPE_V5_9056)) {
            ...
            *f_pos += count;
            ...
            for (i = 0; i < count; i++)
                if (ev_device->devtype & MRF_DEVTYPE_V2P_9030)
                    jtag_9030_load_fpga_byte(ev_device->pLC, ev_device->fpga_conf_data[i]);
                else if (ev_device->devtype & MRF_DEVTYPE_V5_9056)
...
long ev_unlocked_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
#else
int ev_ioctl(struct inode *inode, struct file *filp, unsigned int cmd, unsigned long arg)
#endif
{
    struct mrf_dev *ev_device = (struct mrf_dev *)filp->private_data;
    int ret = 0;
...
int ev_remap_mmap(struct file *filp, struct vm_area_struct *vma) {
    ...
    if (vsize > barlen) {
        printk(KERN_NOTICE DEVICE_NAME ": mmap requested window %08x, device %d window %08x\n", (unsigned int)vsize,
               (unsigned int)ev_device->access_device, (unsigned int)barlen);
        return -EINVAL;
    }
    ...
    result = io_remap_pfn_range(vma, vma->vm_start, physical >> PAGE_SHIFT, vsize, vma->vm_page_prot);
#else
    result = io_remap_page_range(vma, vma->vm_start, physical, vsize, vma->vm_page_prot);
#endif
...
#endif
{
    ...
    if (ev_device->devtype & MRF_DEVTYPE_V2P_9030) {
        ...
        if (!(le32_to_cpu(pLC9056->INTCSR) & PLX9056_INTCSR_LINTI))
            return IRQ_NONE;
    } else if (!(be32_to_cpu(*evr_irq_flag) & EV_IRQ_PCI_DRIVER_ENA))
        return IRQ_NONE;
...
int ev_plx_irq_enable(struct mrf_dev *ev_device) {
    ...
    pLC->INTCSR = __constant_cpu_to_le16(PLX9030_INTCSR_LINTI1_ENA | PLX9030_INTCSR_LINTI1_POL | PLX9030_INTCSR_PCI_IRQENA);
    barrier();
...
int ev_plx_irq_disable(struct mrf_dev *ev_device) {
    ...
    pLC->INTCSR = __constant_cpu_to_le16(PLX9030_INTCSR_LINTI1_ENA | PLX9030_INTCSR_LINTI1_POL);
    barrier();
    return le16_to_cpu(pLC->INTCSR);
}
...
int ev_plx_9056_irq_enable(struct mrf_dev *ev_device) {
    ...
    pLC->INTCSR = __constant_cpu_to_le32(PLX9056_INTCSR_LINTI_ENA | PLX9056_INTCSR_PCI_IRQENA);
    barrier();
